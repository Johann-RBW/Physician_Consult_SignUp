...
- Mirrors the function signatures used by the UI.
...
Tables:
  ...
  signups(
    ...
    ClaimNumber, ClaimType, ClaimQuestion, CreatedAt
  )

  facilitators(email, display_name)

Notes:
- IDs are UUID hex strings (strings).
- DateTime columns stored as ISO 8601 strings (UTC-ish). Keep UI consistent.
"""
...
def _ensure_schema() -> None:
    ...
    # signups (create with claim fields present)
    _exec(
        """
    ...
    # Backfill missing claim columns on older DBs (safe if already exist)
    for ddl in (
...
def list_sessions(active_only: bool = True) -> List[Dict[str, Any]]:
    if active_only:
        rows = _query("SELECT * FROM sessions WHERE Active = 1 ORDER BY StartDateTime ASC;")
    else:
...
# ----------------------
# Facilitator helpers (self-healing: ensure table before every op)
# ----------------------
...
def list_facilitators() -> List[Dict[str, str]]:
    _ensure_facilitators_table()
    rows = _query(
        "SELECT email, COALESCE(display_name, '') AS display_name FROM facilitators ORDER BY email;"
    )
